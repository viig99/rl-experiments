...
class LunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        # New attributes for wind particles
        self.current_wind_mag = 0.0
    ...
    def step(self, action):
        ...
        if self.enable_wind and not (
            ...
            damping_factor = 0.42  # Adjust this factor to smooth transitions
            wind_input = math.sin(0.02 * self.wind_idx) + math.sin(
                math.pi * 0.01 * self.wind_idx
            )
            wind_mag = abs(math.tanh(damping_factor * wind_input) * self.wind_power)
            wind_angle = (
                math.sin(0.01 * self.wind_idx) * math.pi / 4
            )  # Maps wind_idx to a value between -pi and pi

            wind_force_x = wind_mag * np.cos(wind_angle)
            wind_force_y = wind_mag * np.sin(wind_angle)

            self.lander.ApplyForceToCenter(
                (wind_force_x, wind_force_y),
                True,
            )
            self.current_wind_mag = wind_mag
            self.current_wind_angle = wind_angle
    ...
    def _render_wind_speed(self):
        wind_speed_text = (
            f"Wind Speed: {self.current_wind_mag:.2f}"  # Format the wind speed
        )
        text_surface = self.font.render(
            wind_speed_text, True, (108, 127, 80)
        )  # White text
        self.surf.blit(text_surface, (10, 10))  # Position the text on the screen

        wind_angle_text = f"Wind Angle: {self.current_wind_angle * 180/math.pi:.2f} degrees"  # Format the wind angle
        text_surface = self.font.render(wind_angle_text, True, (108, 127, 80))
        self.surf.blit(text_surface, (10, 40))
